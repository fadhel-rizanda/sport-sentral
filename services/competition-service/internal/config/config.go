...
func Load() (*Config, error) {
	...
	metricPort := envConfig.GetEnv("METRIC_PORT", "9006")

	return &Config{
		GRPC: GRPCConfig{
			Port: envConfig.GetEnvInt("GRPC_PORT", 50056), // Default port for competition-service
		},
		Database: DatabaseConfig{
		...
		Nats: messaging.Config{
			...
			StreamName:           "COMPETITION_EVENTS",
			StreamSubjects:       []string{"competition.*.*"},
