...
func (r *userRepository) GetByID(ctx context.Context, id uuid.UUID) (entity.User, error) {
	...
	err := r.db.WithContext(ctx).
		...
		Select("replicated_users.*, statuses.name as status_name").
		Joins("LEFT JOIN statuses ON statuses.id = replicated_users.status_id").
		Where("replicated_users.id = ? AND replicated_users.deleted_at IS NULL", id).
		First(&user).Error
	return user, err
}
